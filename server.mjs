...
const origin = process.env.APP_ORIGIN || (process.env.VERCEL_PROJECT_PRODUCTION_URL ? 'https://'+process.env.VERCEL_PROJECT_PRODUCTION_URL : 'http://127.0.0.1:4173');
const allowedOrigins = new Set([origin, ...(process.env.VERCEL_URL ? ['https://'+process.env.VERCEL_URL] : []), ...(process.env.VERCEL ? ['https://meken.im','https://meken-capital.vercel.app'] : [])]);
...
const db = await openDatabase({dataDir,schema:`
...
CREATE TABLE IF NOT EXISTS applications (id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id), deal_id TEXT NOT NULL REFERENCES deals(id), amount INTEGER NOT NULL, ready_window TEXT, status TEXT NOT NULL DEFAULT 'submitted', created TEXT NOT NULL, UNIQUE(user_id,deal_id));
...
// Databases created before the readiness window exists keep their rows; the column
// is added empty and only new applications are required to carry a window.
for (const statement of ['ALTER TABLE applications ADD COLUMN ready_window TEXT']) {
  try { await db.prepare(statement).run(); }
  catch (error) { if (!/duplicate column|already exists/i.test(error.message)) throw error; }
}
// How soon an investor is ready to transfer the stated amount once a deal opens.
// Declaring interest is never a payment obligation: capital is called only under a signed deal.
const readyWindows = new Map([['3d', 3], ['7d', 7], ['14d', 14]]);
const readyWindowLabel = value => ({'3d': 'в течение 3 дней', '7d': 'в течение недели', '14d': 'в течение двух недель'}[value] || 'срок не указан');
...
async function throttle(key, max) {
  if(db.rateLimit){if(!await db.rateLimit(hash(key),max))fail(429,'Слишком много попыток. Повторите через 15 минут.');return;}
...
async function session(req) {
  ...
  if (!raw) return null;
  return (await db.prepare('SELECT u.id,u.email,u.name,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token=? AND s.expires>?').get(hash(raw), Date.now())) || null;
}
...
async function api(req,res,url) {
  const u=await session(req), method=req.method, p=url.pathname;
  if (!['GET','HEAD'].includes(method)) {
    if (!allowedOrigins.has(req.headers.origin) || req.headers['x-meken-request'] !== '1') fail(403,'Недопустимый источник запроса');
  ...
  if (p==='/api/logout' && method==='POST') {
    const raw=/(?:^|;\s*)meken_session=([a-f0-9]+)/.exec(req.headers.cookie||'')?.[1];
    if(raw) (await db.prepare('DELETE FROM sessions WHERE token=?').run(hash(raw)));
  ...
  if (p==='/api/applications' && method==='GET') {
    requireUser(u);return json(res,{applications:(await db.prepare('SELECT a.*,d.payload FROM applications a JOIN deals d ON d.id=a.deal_id WHERE a.user_id=? ORDER BY a.created DESC').all(u.id)).map(a=>({...a,deal:JSON.parse(a.payload),payload:undefined}))});
  }
  if (p==='/api/applications' && method==='POST') {
    requireUser(u); const b=await body(req); const row=(await db.prepare('SELECT * FROM deals WHERE id=? AND published=1').get(b.dealId||''));if(!row)fail(404,'Проект недоступен');
    ...
    if(!readyWindows.has(b.readyWindow))fail(400,'Укажите срок, в который готовы внести сумму по первому требованию');
    ...
    const aid=id();(await db.prepare('INSERT INTO applications(id,user_id,deal_id,amount,ready_window,status,created) VALUES(?,?,?,?,?,?,?)').run(aid,u.id,d.id,amount,b.readyWindow,'submitted',now()));await audit(u,'application-consent-v1',aid);await notify(u.id,'Заявка на «'+d.title+'» получена: готовность внести сумму '+readyWindowLabel(b.readyWindow)+' после открытия сделки. Это заявка на обсуждение, денежные средства не списаны.');return json(res,{ok:true},201);
  }
  ...
  if (p==='/api/documents' && method==='GET') {
    ...
    if(u.role!=='admin'&&!(await db.prepare("SELECT id FROM applications WHERE user_id=? AND deal_id=? AND status='approved'").get(u.id,deal)))fail(403,'Документы доступны после допуска к проекту');
    return json(res,{documents:(await db.prepare('SELECT id,name,kind,created FROM documents WHERE deal_id=?').all(deal))});
  }
  if (p.startsWith('/api/documents/') && method==='GET') {
    requireUser(u);const d=(await db.prepare('SELECT * FROM documents WHERE id=?').get(p.split('/').pop()));if(!d)fail(404,'Документ не найден');
    if(u.role!=='admin'&&!(await db.prepare("SELECT id FROM applications WHERE user_id=? AND deal_id=? AND status='approved'").get(u.id,d.deal_id)))fail(403,'Нет доступа');
    await audit(u,'document-download',d.id);res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="document.pdf"; filename*=UTF-8''${encodeURIComponent(d.name)}`,'Cache-Control':'no-store'});return res.end(Buffer.from(d.content));
  }
  if(p==='/api/polls'&&method==='GET') {
    requireUser(u);return json(res,{polls:(await db.prepare("SELECT p.*,v.answer FROM polls p JOIN applications a ON a.deal_id=p.deal_id AND a.user_id=? AND a.status='approved' LEFT JOIN votes v ON v.poll_id=p.id AND v.user_id=? ORDER BY p.closes DESC").all(u.id,u.id))});
  }
  if(p==='/api/vote'&&method==='POST') {
    requireUser(u);const b=await body(req);const poll=(await db.prepare('SELECT * FROM polls WHERE id=?').get(b.pollId||''));
    if(!poll||poll.closes<=now())fail(400,'Голосование закрыто');if(!['yes','no','abstain'].includes(b.answer))fail(400,'Выберите ответ');
    if(!(await db.prepare("SELECT id FROM applications WHERE user_id=? AND deal_id=? AND status='approved'").get(u.id,poll.deal_id)))fail(403,'Нет доступа');
    if((await db.prepare('SELECT answer FROM votes WHERE poll_id=? AND user_id=?').get(poll.id,u.id)))fail(409,'Голос уже принят');
    (await db.prepare('INSERT INTO votes VALUES(?,?,?,?)').run(poll.id,u.id,b.answer,now()));await audit(u,'vote',poll.id);return json(res,{ok:true});
  }
  ...
  if(p.startsWith('/api/admin/')) {
    ...
    if(p==='/api/admin/deals'&&method==='POST') {
      ...
      (await db.prepare('INSERT INTO deals(id,payload,published) VALUES(?,?,0) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload,published=0,version=deals.version+1').run(did,JSON.stringify(d)));await audit(u,'deal-save-unpublished',did);return json(res,{id:did});
    }
    if(p==='/api/admin/publish'&&method==='POST') {
      const b=await body(req), row=(await db.prepare('SELECT * FROM deals WHERE id=?').get(b.id||''));if(!row)fail(404,'Проект не найден');
      if(b.published===true) {
        const kinds=(await db.prepare('SELECT kind FROM documents WHERE deal_id=?').all(row.id)).map(d=>d.kind);
      ...
      (await db.prepare('UPDATE deals SET published=?,version=version+1 WHERE id=?').run(b.published===true?1:0,row.id));await audit(u,b.published?'publish':'unpublish',row.id);return json(res,{ok:true});
    }
    if(p==='/api/admin/documents'&&method==='POST') {
      const b=await body(req);if(!(await db.prepare('SELECT id FROM deals WHERE id=?').get(b.dealId||'')))fail(404,'Проект не найден');
      if(!['asset','finance','contract','sharia','report'].includes(b.kind))fail(400,'Выберите тип документа');
      const content=Buffer.from(typeof b.base64==='string'?b.base64:'','base64');if(content.length>3*1024*1024||content.subarray(0,5).toString()!=='%PDF-')fail(400,'Загрузите PDF до 3 МБ');
      const name=text(b.name,180);if(!name)fail(400,'Укажите название');const did=id();(await db.prepare('INSERT INTO documents VALUES(?,?,?,?,?,?)').run(did,b.dealId,name,b.kind,content,now()));await audit(u,'document-upload',did);return json(res,{ok:true});
    }
    if(p==='/api/admin/application'&&method==='POST') {
      const b=await body(req);if(!['review','approved','declined'].includes(b.status))fail(400,'Недопустимый статус');const a=(await db.prepare('SELECT * FROM applications WHERE id=?').get(b.id||''));if(!a)fail(404,'Заявка не найдена');
      (await db.prepare('UPDATE applications SET status=? WHERE id=?').run(b.status,a.id));await notify(a.user_id,'Статус заявки: '+({review:'на проверке',approved:'допуск к документам открыт',declined:'отклонена'}[b.status]));await audit(u,'application-'+b.status,a.id);return json(res,{ok:true});
    }
    if(p==='/api/admin/polls'&&method==='POST') {
      const b=await body(req);if(!(await db.prepare('SELECT id FROM deals WHERE id=?').get(b.dealId||'')))fail(404,'Проект не найден');const question=text(b.question,500),dt=new Date(b.closes);
      if(!question||!Number.isFinite(dt.getTime())||dt.getTime()<=Date.now())fail(400,'Укажите вопрос и будущую дату');
      const pid=id();(await db.prepare('INSERT INTO polls VALUES(?,?,?,?)').run(pid,b.dealId,question,dt.toISOString()));await audit(u,'poll-create',pid);return json(res,{ok:true});
    }
...
export async function handler(req,res){
  ...
  try {
    const url=new URL(req.url,'http://localhost');if(url.pathname.startsWith('/api/')){
      if(['GET','HEAD'].includes(req.method))return await api(req,res,url);
      let response;
      const headers={};
      const buffered={setHeader:(k,v)=>{headers[k]=v;},writeHead:(status,h={})=>{response={status,headers:{...headers,...h}};},end:content=>{response={status:200,headers,...response,content};}};
      await db.transaction(()=>api(req,buffered,url));
      res.writeHead(response.status,response.headers);return res.end(response.content);
    }
...
if (!process.env.VERCEL) server.listen(Number(process.env.PORT||4173),process.env.HOST||'127.0.0.1',()=>console.log(`Meken ready: ${origin}`));
if (!process.env.VERCEL) process.on('SIGTERM',()=>server.close(async()=>{await db.close();process.exit(0);}));
