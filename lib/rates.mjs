...
export function createRateService({ read, write, fetcher = fetch, clock = Date.now }) {
  ...
  async function cached() { const v=await read();return v?.rateSource==='НБКР'?v:null; }
  return async function rates() {
    if(clock() >= nextCheck && !pending) pending=(async()=>{
      try {
        ...
        await write(value); failed=false;nextCheck=clock()+3600000;
      } catch {failed=true;nextCheck=clock()+300000;}
      finally {pending=null;}
    })();
    if(pending)await pending;
    const v=await cached();return v?{...v,rateStale:failed}:{rateUnavailable:true};
  };
}
