...
test('server: persistent accounts, publication gates, private documents, decisions and revocation',async t=>{
 ...
 assert.equal((await req('applications','POST',{dealId:did,amount:10,readyWindow:'3d',consent:true},investor)).status,400);
 const noWindow=await req('applications','POST',{dealId:did,amount:100000,consent:true},investor);
 assert.equal(noWindow.status,400);assert.match(noWindow.data.error,/срок/i);
 assert.equal((await req('applications','POST',{dealId:did,amount:100000,readyWindow:'month',consent:true},investor)).status,400);
 assert.equal((await req('applications','POST',{dealId:did,amount:100000,readyWindow:'7d',consent:false},investor)).status,400);
 assert.equal((await req('applications','POST',{dealId:did,amount:100000,readyWindow:'7d',consent:true},investor)).status,201);
 assert.equal((await req('applications','POST',{dealId:did,amount:100000,readyWindow:'7d',consent:true},investor)).status,409);
 assert.equal((await req('applications','GET',undefined,investor)).data.applications[0].ready_window,'7d');
 assert.match((await req('notifications','GET',undefined,investor)).data.notifications[0].message,/в течение недели/);
